...
// Submenu Administração (apenas admin)
...
const auditMenuItems = [
  ...
  { icon: PackageOpen, label: "Mov. Estoque", path: "/auditoria-movimentacoes", roles: ["admin"] },
];
...
const SALON_SUBMENU_KEY = "salon-submenu-expanded";
...
function DashboardLayoutContent({
  ...
  const [adminExpanded, setAdminExpanded] = useState(() => {
    const saved = localStorage.getItem(ADMIN_SUBMENU_KEY);
    return saved === 'true';
  });
  ...
  // Estado do submenu do salão
  const [salonExpanded, setSalonExpanded] = useState(() => {
    const saved = localStorage.getItem(SALON_SUBMENU_KEY);
    return saved === 'true';
  });
  ...
  // Verifica se algum item do salão está ativo
  const isSalonActive = filteredSalonItems.some(item => item.path === location);
  
  // Encontra o item ativo atual
  const allItems = [...filteredMainItems, ...filteredAnalysisItems, ...filteredFinanceItems, ...filteredAccountingItems, ...filteredAdminItems, ...filteredAuditItems, ...filteredSalonItems];
  ...
  useEffect(() => {
    localStorage.setItem(SALON_SUBMENU_KEY, salonExpanded.toString());
  }, [salonExpanded]);
  ...
  // Expande automaticamente o submenu se um item de administração estiver ativo
  useEffect(() => {
    if (isAdminActive && !adminExpanded) {
  ...
  // Expande automaticamente o submenu do salão se ativo
  useEffect(() => {
    if (isSalonActive && !salonExpanded) {
                ...
                <DropdownMenuTrigger asChild>
                  <button className="flex items-center gap-3 w-full rounded-lg border px-3 py-3 text-left transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-ring" style={hasCustomTheme ? { borderColor: companyTheme.borderColor, backgroundColor: companyTheme.accentHover } : { borderColor: 'var(--primary-20)', backgroundColor: 'var(--primary-5)' }}>
                    {activeCompany?.companyLogoUrl ? (
                      <img src={activeCompany.companyLogoUrl} alt="" className="h-14 w-14 rounded-lg object-contain shrink-0" style={{ filter: 'drop-shadow(0 0 1px rgba(0,0,0,0.8)) drop-shadow(0 0 2px rgba(0,0,0,0.6)) drop-shadow(0 1px 4px rgba(0,0,0,0.5))' }} />
                    ) : (
                ...
                <>
                  ...
                  <SidebarMenuItem>
                    <SidebarMenuButton
                      ...
                      className={`h-10 transition-all font-medium ${isAdminActive ? "bg-accent text-accent-foreground" : ""}`}
                    >
                      <Settings className={`h-4 w-4 ${isAdminActive ? "text-primary" : ""}`} />
                ...
                <>
                  ...
                  <SidebarMenuItem>
                    <SidebarMenuButton
                      ...
                      tooltip="Salão"
                      className={`h-10 transition-all font-medium ${isSalonActive ? "bg-accent text-accent-foreground" : ""}`}
                    >
                      <UtensilsCrossed className={`h-4 w-4 ${isSalonActive ? "text-primary" : ""}`} />
                      <span className="flex-1">Salão</span>
                      {!isCollapsed && (
                        salonExpanded ? (
                          <ChevronDown className="h-4 w-4 text-muted-foreground" />
                        ) : (
                  ...
                  {salonExpanded && !isCollapsed && (
                    <div className="ml-4 border-l pl-2" style={hasCustomTheme ? { borderColor: companyTheme.separatorColor } : { borderColor: 'var(--border)' }}>
                      {visibleSalonItems.map(item => {
                        const isActive = location === item.path;
                        return (
                          <SidebarMenuItem key={item.path}>
                            <SidebarMenuButton
                  ...
                  {isCollapsed && visibleSalonItems.map(item => {
                    const isActive = location === item.path;
                    return (
                      <SidebarMenuItem key={item.path}>
                        <SidebarMenuButton
